...
RubiksCubeSolver.prototype.applyMove = function(move, inState) {
  ...
  while (turns-- > 0) {
    ...
    for (var i = 0; i < 8; i++) {
      ...
      var killer = affectedCubies[face][(i & 3) == 3 ? i - 3 : i + 1] +
        isCorner * 12;
      var orientationDelta = (i < 4) ? (face > 1 && face < 4) : (face < 2) ?
        0 : 2 - (i & 1);
...
RubiksCubeSolver.prototype.setState = function(cube) {
  ...
  var goal = ["UF", "UR", "UB", "UL", "DF", "DR", "DB", "DL", "FR", "FL",
...
RubiksCubeSolver.prototype.prepareSolution = function() {
  ...
  for (var i = 0; i < moves.length - 2; i++) {
    if (moves[i][0] == moves[i + 2][0] && opposites[moves[i + 1][0]] == moves[
        i][0]) {
      temp = moves[i + 2];
      moves[i + 2] = moves[i + 1];
      moves[i + 1] = temp;
